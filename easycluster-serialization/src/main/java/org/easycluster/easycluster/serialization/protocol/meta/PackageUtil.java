...
public class PackageUtil {
	...
	public static String[] findClassesInPackage(String packageName, List<String> included, List<String> excluded) throws IOException {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("PackageUtil: getResources: " + dirs + ", hasMoreElements:" + dirs.hasMoreElements());
		}
			...
			if ("file".equals(protocol)) {
				...
				while (entries.hasMoreElements()) {
					...
					if (name.startsWith(packageDirName)) {
						...
						if (logger.isDebugEnabled()) {
							logger.debug("PackageUtil: Package name is " + packageName);
						}
						...
						if ((idx != -1) || recursive) {
							...
							if (name.endsWith(".class") && !entry.isDirectory()) {
								...
								if (logger.isDebugEnabled()) {
									logger.debug("PackageUtil: Found class " + className + ", seeing it if it's included or excluded");
								}
	...
	private static void findClassesInDirPackage(String packageName, List<String> included, List<String> excluded, String packagePath, final boolean recursive,
			List<String> classes) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("PackageUtil: Looking for test classes in the directory: " + dir);
		}
